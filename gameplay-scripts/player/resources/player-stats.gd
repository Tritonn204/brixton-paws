extends Resource

class_name PlayerStats

export(float) var acceleration = 168.75
export(float) var deceleration = 1800
export(float) var friction = 168.75
export(float) var slope_factor = 600
export(float) var top_speed = 360
export(float) var dash_speed = 600
export(float) var min_speed_to_brake = 240

export(float) var min_speed_to_roll = 60
export(float) var min_speed_to_detach = 360
export(float) var unroll_speed = 30
export(float) var slope_roll_up = 300.25
export(float) var slope_roll_down = 1125
export(float) var roll_deceleration = 600

export(float) var roll_friction = 84.375
export(float) var air_acceleration = 337.5
export(float) var gravity = 787.5
export(float) var max_jump_height = 390
export(float) var min_jump_height = 240
export(float) var slide_angle = 45

export(float) var cling_slide_angle = 75
export(float) var cling_friction = 1000
export(float) var cling_slope_factor = 110
export(float) var cling_acceleration = 268.75
export(float) var fall_angle = 80

export(float) var min_speed_to_fall = 150
export(float) var control_lock_duration = 0.5
export(float) var control_lock_friction = 200
export(float) var wall_grace_period = 0.4
export(float) var wall_jump_direction_blend := 0.5
